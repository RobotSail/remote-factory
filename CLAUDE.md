...
The factory uses Claude Code (`claude` CLI) as its runner backend (`factory/runners/`). Third-party runners can be registered via Python entry points (`factory.runners` group).

**Runner selection:** Default is `claude`. Set `FACTORY_RUNNER` env var or pass `--runner` to individual commands if using a plugin runner.
