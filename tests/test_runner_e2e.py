...
_DRY_RUN_VARS = ["FACTORY_CODEX_DRY_RUN", "FACTORY_OPENCODE_DRY_RUN"]
...
def test_get_available_runners_includes_all_builtins() -> None:
    """get_available_runners includes claude runner."""
    runners = get_available_runners()
...
def _cli_env() -> dict[str, str]:
    """Build subprocess env for CLI tests."""
    env = os.environ.copy()
...
def test_factory_runners_list_all_present() -> None:
    """factory runners list --json shows registered runners with correct metadata."""
    result = subprocess.run(
